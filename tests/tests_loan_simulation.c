...
void test_IsComplete_NotFinished(void)
{
    ...
    SimulationState state = {.current_balance = money_from_major(1000), .current_month = 5};
...
{
    LoanDefinition loan = {.term_months = 12};
    SimulationState state = {.current_balance = MONEY_ZERO, .current_month = 5};
...
{
    LoanDefinition loan = {.term_months = 12};
    SimulationState state = {.current_balance = money_from_major(100), .current_month = 12};
    TEST_ASSERT_TRUE(loan_simulation_is_complete(&loan, &state));
}
...
{
    LoanDefinition loan = {
        .principal = money_from_major(1000), .term_months = 2, .type = LOAN_EQUAL_INSTALLMENTS};
...
{
    LoanDefinition loan = {
        .principal = money_from_major(1000), .term_months = 10, .type = LOAN_EQUAL_INSTALLMENTS};
...
void test_Run_ValidateInputs_NullPointers(void)
{
    ...
    TEST_ASSERT_EQUAL(FINANCE_ERR_INVALID_ARGUMENT,
                      run_loan_simulation(&loan, &market, &config, NULL));
}
...
void test_Run_ValidateInputs_InvalidPrincipal(void)
{
    ...
    TEST_ASSERT_EQUAL(FINANCE_ERR_INVALID_PRINCIPAL,
                      run_loan_simulation(&loan, &market, &config, &result));
}
...
void test_Run_ValidateInputs_InvalidRates(void)
{
    ...
    TEST_ASSERT_EQUAL(FINANCE_ERR_INVALID_RATE,
                      run_loan_simulation(&loan, &market, &config, &result));
}
...
void test_Run_HappyPath(void)
{
    ...
    LoanDefinition loan = {.principal = money_from_major(10000),
                           .term_months = months,
